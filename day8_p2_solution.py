    ...
    for pair in antenna_pairs:
        ...
        if row_diff == 0:
            for col in range(grid_col_count):
                antinode_positions.append(a1[0], col)
        else:
            if col_diff != 0:
                tan = row_diff / col_diff
                for row in range(grid_row_count):
                    col = ((row - a1[0]) / tan) + a1[1]
                    if col.is_integer() and int(col) >= 0 and int(col) < grid_col_count:
                        antinode_positions.append((row, int(col)))
            else:
                for row in range(grid_row_count):
                    col = a1[1]
                    antinode_positions.append((row, int(col)))
