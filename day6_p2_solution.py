...
all_traveled_positions_list = []
traveled_positions = []
is_travel_completed =  False
travel_direction = "north"

# until travel is completed(until guards is out of grid), call the walk_grid function and add visitied positions into a list
while not is_travel_completed:
    ...
    all_traveled_positions_list.append(traveled_positions)


# filter visited positions and remove duplicates.
filtered_all_traveled_postions = list(set(all_traveled_positions))

# get a deep copy of visited positions.
all_path_positions = copy.deepcopy(filtered_all_traveled_postions)

place_to_put_barriers = []

# find guard's stating position.
sr, sc =  next((idx, sublist.index("^")) for idx,sublist in enumerate(grid) if "^" in sublist)

# remove gurd's stating positon because no obstruction will be put here.
all_path_positions.remove((sr,sc))

# for every postions in guard's path, put an obstruction and check if it causes a loop.
# if it causes a loop, save obstruction position in a list.
for idx, tp in enumerate(all_path_positions):
    all_traveled_positions_list = []
    grid[tp[0]][tp[1]] = "#"
    is_travel_completed = False
    travel_direction = "north"
    starting_row, starting_col =  sr, sc
    counter = 0
    while not is_travel_completed:
        starting_row, starting_col, traveled_positions, travel_direction, is_travel_completed = walk_grid(starting_row, starting_col, travel_direction, num_of_rows, num_of_columns, grid)

        if traveled_positions in all_traveled_positions_list:
                place_to_put_barriers.append((tp[0],tp[1]))
                break
        else:
                all_traveled_positions_list.append(traveled_positions)
    grid[tp[0]][tp[1]] = "."

print(f'Different positions to choose for obstruction: {len(list(set(place_to_put_barriers)))}')
